...
func _load_object(path: String):
	...
	for _i in min_load_amount:
		var object = resource.instantiate()
...
func _clean_return_queue():
	while len(_return_queue) != 0:
		...
		if"res://ball.tscn" in object_pool:
			object_pool["res://ball.tscn"].append(object)
		else:
			object_pool["res://ball.tscn"] = [object]
